...
public class EditProfilMasjidActivity extends AppCompatActivity {
    ...
    private String valid1, namaMasjid, alamtMasjid, tahunBerdiri, dayaTampung;
...
//                masjid.setMlongitude(place.getLatLng().longitude);
                ...
                try {
                    startActivityForResult(builder.build(EditProfilMasjidActivity.this), PICK_ADDRESS);
    ...
    public void setupUI() {
        btnCancel = findViewById(R.id.btn_cancel_edit_data_masjid);
        btnSimpan = findViewById(R.id.btn_simpan_data_masjid);
        etNamaMasjid = findViewById(R.id.field_edit_nama_masjid);
        tv_alamat = findViewById(R.id.field_edit_alamat_masjid);
        etTahunBerdiri = findViewById(R.id.field_edit_tahun_berdiri_masjid);
        etDayaTampung = findViewById(R.id.field_edit_daya_tampung_masjid);
        tv_fotoMasjid = findViewById(R.id.field_edit_foto_masjid);
    }
...
//                loginAccount(view, isValid);
    ...
    public void updateProfilMasjid (boolean isValid) {
        if (isValid) {
            ...
            profilMasjidViewModel.uploadImage(SharedPrefManager.getInstance(getApplicationContext()).getUser().getMidmasjid(), getStringImage(bitmap));
            ...
            profilMasjidViewModel.updateProfilMasjid(masjid);
            profilMasjidViewModel.getData().observe(this, profileMasjid -> {
                if (profileMasjid != null && valid1 == "true"){
                    Toast.makeText(getApplicationContext(), "Profil berhasil untuk di update", Toast.LENGTH_SHORT).show();
                }else {
                    Toast.makeText(getApplicationContext(), "Profil gagal untuk di update", Toast.LENGTH_SHORT).show();
                }
            });
        }
    }
    ...
    public void getData(){
        ...
        if(getIntent().hasExtra("nama_masjid")
                ...
                && getIntent().hasExtra("daya_tampung"))
        {
            ...
            alamtMasjid = bundle.getString("alamat_masjid");
            tahunBerdiri = bundle.getString("tahun_berdiri");
            dayaTampung = bundle.getString("daya_tampung");
        }
    }

    public void setData(){
        etNamaMasjid.setText(namaMasjid);
        tv_alamat.setText(alamtMasjid);
        etDayaTampung.setText(dayaTampung);
        etTahunBerdiri.setText(tahunBerdiri);
    }
}